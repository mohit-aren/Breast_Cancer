...
def prep_data1(mode, autoencoder):
    ...
    for index, image in enumerate(images_path):
        ...
        imageio.imwrite('predict_segnet_c_'+ folder_path + ' ('+ str(index) + ')' + '.jpg', labeled1.astype('uint8'))
...
autoencoder.summary()
start = time.time()
autoencoder.load_weights('model_5l_weight_compress_mini.hdf5')
print('Start')
prep_data1('test', autoencoder)
stop = time.time()
print(stop-start)
