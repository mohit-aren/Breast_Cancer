...
def prep_data1(mode, autoencoder):
    ...
    for index, image in enumerate(images_path):
        ...
        new_im = img1.crop((start, s_h, start+320, s_h+320))
        imageio.imwrite('benign'+ folder_path + ' ('+ str(index) + ')' + '.jpg', new_im)
        ...
        imageio.imwrite('predict_fcn_compress'+ folder_path + ' ('+ str(index) + ')' + '.jpg', labeled1.astype('uint8'))
...
olayer8_filters = 301
olayer9_filters = 321
olayer10_filters = 276
olayer11_filters = 297
olayer12_filters = 302
olayer13_filters = 304
olayer14_filters = 2112
olayer15_filters = 2071
...
model_final.summary()
start = time.time()
model_final.load_weights('FCN_pruned_weights_1.h5')
prep_data1('test', model_final)
stop = time.time()
